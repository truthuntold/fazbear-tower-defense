    ...
    >
      ...
      <svg className="absolute inset-0 pointer-events-none z-50" width={GRID_SIZE * cellSize} height={GRID_SIZE * cellSize}>
        {projectiles.map(p => {
           ...
           const curX = p.start.x * cellSize + cellSize/2 + (p.end.x - p.start.x) * cellSize * progress;
           const curY = p.start.y * cellSize + cellSize/2 + (p.end.y - p.start.y) * cellSize * progress;
           return (
             <g key={p.id}>
